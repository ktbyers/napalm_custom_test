...
def test_facts(napalm_connect):
    ...
    assert (
        isinstance(napalm_facts["serial_number"], six.string_types)
        and napalm_facts["serial_number"]
    )
    assert "nxos1" in napalm_facts["hostname"]
    assert napalm_facts["fqdn"] == "nxos1.twb-tech.com"
    assert "NX-OSv" in napalm_facts["model"]
    assert (
        "Ethernet2/1" in napalm_facts["interface_list"]
        or "Eth2/1" in napalm_facts["interface_list"]
    )
